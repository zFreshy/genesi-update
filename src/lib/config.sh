...
if [ -f "${config_file}" ]; then
	...
	# Check the "KeepOldPackages" option in genesi-update.conf
	# shellcheck disable=SC2034
	old_packages_num=$(grep -E '^[[:space:]]*KeepOldPackages[[:space:]]*=[[:space:]]*[0-9]+[[:space:]]*$' "${config_file}" 2> /dev/null | awk -F '=' '{print $2}' | tr -d '[:space:]')

	# Check the "KeepUninstalledPackages" option in genesi-update.conf
	# shellcheck disable=SC2034
	...
	# Check the "ColorblindMode" option in genesi-update.conf
	# shellcheck disable=SC2034
	colorblind_mode=$(grep -Eq '^[[:space:]]*ColorblindMode[[:space:]]*$' "${config_file}" 2> /dev/null && echo "-cb")
fi

# Set the default / fallback value for options that require it (if the genesi-update.conf configuration file doesn't exists, if the concerned option is commented or if the set value is invalid) 
